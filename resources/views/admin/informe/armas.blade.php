...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0">Armas decomisadas</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('index')}}">Inicio</a></li>
                <li class="breadcrumb-item active">Armas decomisadas</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
...
<div class="card">
    <div class="card-header d-flex">
        <h3 class="card-title">Listado de decomisos de armas</h3>
        
    </div>
    <!-- /.card-header -->
    <div class="card-body" >
        @include('admin.partials.fechas')
        <table id="arma-table" class="table table-bordered table-striped table-hover" style="width:100%">
            <thead>
                <tr>
                    <th>Descripción</th>
...
<script>
    ...
    var table=$('#arma-table').DataTable({
        ...
        order:[[4, "desc"]],
        ajax:{
            "url": "{{ url('decomisoarma')}}", ////////acción que llena la tabla/////////
    ...
    function imprimirDOCPDF(datos){/////////////////función para exportar a PDF/////////////////
        ...
        datos.forEach(function(decomiso_arma) {
            var deco=[ decomiso_arma.arma, decomiso_arma.cantidad, decomiso_arma.departamento, decomiso_arma.municipio, decomiso_arma.fecha ];
            registros.push(deco);
        });
        ...
        pdfMake.createPdf(docDefinition).download('Listado_de_decomisos_de_armas_Captis.pdf');
        imprimirPDF = false;
        imprimir = false;
    }
    ...
    function imprimirDOCXLS(datos){/////////////////////////función para exportar a Excell/////////////////////
        ...
        wb.Props = {
            Title: "Captis",
            Subject: "Listado de decomisos de armas Captis",
            Author: "Captis",
            //CreatedDate: fecha
            };
        ...
        if(imprimir==true){
            console.log(datos);
            datos.forEach(function(decomiso_arma) {
                var deco={ Descripción: decomiso_arma.arma, Cantidad: decomiso_arma.cantidad, Departamento: decomiso_arma.departamento, Municipio: decomiso_arma.municipio, Fecha: decomiso_arma.fecha};
                rows.push(deco);
            });
        }
        ...
        saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'Listado_de_armas_decomisadas_Captis.xlsx');
